...
all_counts <- readRDS("documents/presentation/data/growth_counts_4.rds")
...
# export plot as file
ggsave(
  paste0("growth-benefit-ON-disturbance-4.pdf"),
...
benefit <- readRDS("documents/presentation/data/growth_counts_4.rds") %>%
  filter(dist_lvl == 0)
...
benefit %>% ggplot(
  ...
  labs(
    caption = "\n\nNow polyploids have increased growth rate.\nThere is still no disturbance."
  )

# export plot as file
ggsave(
  paste0("growth-benefit_ON-1.pdf"),
  path = "documents/presentation/plots", device = "pdf",
  width = 11.69, height = 8.27, units = "in",
  dpi = "retina",
  title = "Gigas Effects of Being Polyploid"
)
